...
#### constructors, and parsing entries
    def __init__(self):
        ...
        self.__dirDict = {
            ...
            '.string'            : self.__dir_string,  # emit string
...
#NEWMY
                regnum = section.extra.get('regnum')
    ...
    def __parseKernelText(self, section, line_start, line_end):
        ...
        for lineidx in range(line_start, line_end):
            ...
            c_icode_s = self.eval_str_code(c_icode_s) #NEW:: calculate values like (0x10 + 0x20)
...
#NEWMY
                        if ".rela.debug_frame" in self.__mSectionDict:
                            rel = CuAsmRelocation(fixup.section, fixup.offset, symname, relsymid, reltype=reltype, reladd=True)
                        else:
                            rel = CuAsmRelocation(fixup.section, fixup.offset, symname, relsymid, reltype=reltype, reladd=None)
...
#NEWMY
    def __dir_size(self, args):
        ...
        if len(args) > 2:
            self.__mSymbolDict[symbol].value = int(args[2])
    ...
    def __evalInstructionFixup(self, section, offset, s):
        ...
        if r2:
            ...
            if section.name == clabel.section.name: # hardcoded target in current section
                ...
                #NEW::
                if clabel.name.startswith(".relN_"):
                    val = offset - val
                    val = val & 0xFFFFFFFF  # convert to unit32
                if clabel.name.startswith(".relP_"):
                    val = val - offset
                    val = val & 0xFFFFFFFF  # convert to unit32
...
#NEWMY
    def __emitString(self, bs):
        '''emit string to current section.'''
        s = result = ", ".join(bs)
        s = s.strip().strip('"')
        b = s.encode("utf-8") + b"\x00"
        self.__mCurrSection.emitBytes(b)
...
#### Help functions to display some internal states.
    ...
    def stripComments(s):
        ...
        #NEWMY
        s = s.strip()
        s = s.split("?", 1)[0] #remove additional things like ?trans7
        s = s.split("&", 1)[0] #remove additional things like  &wr=0x0   &req={0}
        return s
#######################################################################################
    def eval_str_code(self, code_line_str: str) -> str:
        import ast
        import operator
        ...
        def eval_expr(expr: str) -> float | int:
            ...
            """
            # Parse to AST in expression mode
            ...
            # Convert exact floats like 86.0 to int 86
            if isinstance(result, float) and result.is_integer():
                return int(result)
            return result

        raw_parts = code_line_str.split(",")
        new_parts = []
        for part in raw_parts:
            spart = part.strip()
            if (not spart) or ('A' <= spart[0] <= 'Z') or ('a' <= spart[0] <= 'z'):
                new_parts.append(spart)
                continue
            try:
                spart = hex(int(eval_expr(spart)))
            except Exception as e:
                spart = spart #nothing
            new_parts.append(spart)
        # Join back with normalized separator
