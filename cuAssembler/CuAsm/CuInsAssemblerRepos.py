...
class CuInsAssemblerRepos():
    ...
    def assemble(self, addr, s, precheck=True, showCandidates=True):
        ...
        ###########################################################
        #NEW:: some commands we handle in another way, fuck matrices and other shit
        code = self.try_new_assemble_instruction(ins_key, ins_vals, ins_modi)
        if code >= 0:
            return code
        ###########################################################
    ...
    #NEW::
    def try_new_assemble_instruction(self, ins_key, ins_vals, ins_modi):
        return NewOpsHandler.check_new_ops(Config.SM_VER, ins_key, ins_vals, ins_modi)
