...
class CubinFile():
    ...
    def loadCubin(self, binname):
        ...
        with BytesIO(self.__mCubinBytes) as bio:
            ...
            # NEWMY
            if sm_version < 50:
                vsm_version = (self.__mELFFileHeader['e_flags']>>8)&0xff
                sm_version = vsm_version
    ...
    def __writeFileHeaderAsm(self, stream, ident='\t'):
        ...
        #NEWMY
        if isinstance(fheader['e_version'], str):
    ...
    def __writeImplicitSectionAsm(self, stream, secname):
        ...
        if secname == '.shstrtab' or secname == '.strtab':
            ...
            for rel in rel_section.iter_relocations():
                ...
                irel += 1
#NEWMY
        elif secname.startswith('.nv.capmerc.text'):
            stream.write('\t.section  "%s", %s, %s\n'%(secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
        elif secname.startswith('.nv.merc.debug_frame'):
            stream.write('\t.section  "%s", %s, %s\n' % (secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
        elif secname.startswith('.nv.merc.nv.info'):
            stream.write('\t.section  "%s", %s, %s\n' % (secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
        elif secname.startswith('.nv.merc.rela.debug_frame'):
            stream.write('\t.section  "%s", %s, %s\n' % (secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
        elif secname.startswith('.nv.merc.nv.shared.reserved.0'):
            stream.write('\t.section  "%s", %s, %s\n' % (secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
        elif secname.startswith('.nv.merc.symtab'):
            stream.write('\t.section  "%s", %s, %s\n' % (secname, header['sh_flags'], header['sh_type']))
            stream.write('\t// ... \n')
            self.__writeSectionHeaderAsm(stream, secname, header)
            stream.write(bytes2Asm(data))
    ...
    def disassembleCubin(binname):
        ...
        asmtext = check_output([Config.NVDISASM_PATH, "-ndf", binname]).decode()
