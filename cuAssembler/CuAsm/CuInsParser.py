...
p_ConstTrDict89 = {r'(?<!\.)\bRZ\b' : 'R255', r'\bURZ\b' : 'UR63',
                r'\bPT\b' : 'P7', r'\bUPT\b' : 'UP7', r'\bQNAN\b' : 'NAN'} #, r'\.reuse\b':''

#right now we have only limited support of UR for blackwell, same 63 instead of 255
# so still use UR63 for URZ and set UR63 to zero in code!
# otherwise we will have to implement a lot of instructions
p_ConstTrDict120 = {r'(?<!\.)\bRZ\b' : 'R255', r'\bURZ\b' : 'UR63',
                r'\bPT\b' : 'P7', r'\bUPT\b' : 'UP7', r'\bQNAN\b' : 'NAN'} #, r'\.reuse\b':''
...
class CuInsParser():
    ...
    def __constTr(self, s):
        ...
        if self.m_Arch.mVersion == 120:
            for cm in p_ConstTrDict120:
                s = re.sub(cm, p_ConstTrDict120[cm], s)
        else:
            for cm in p_ConstTrDict89:
                s = re.sub(cm, p_ConstTrDict89[cm], s)
